...
async function maybeExecuteCase(pack, item) {
  if (!pack.execution || !item.input) return item;
  if (pack.execution.provider !== 'openai') throw new Error(`Unsupported execution provider: ${pack.execution.provider}`);
  const system = item.system || pack.execution.system;
  const [baselineOutput, candidateOutput] = await Promise.all([
    pack.execution.baselineModel
      ? runOpenAICompletion(pack.execution, { model: pack.execution.baselineModel, system, input: item.input })
      : Promise.resolve(item.baselineOutput || ''),
    pack.execution.candidateModel
      ? runOpenAICompletion(pack.execution, { model: pack.execution.candidateModel, system, input: item.input })
...
async function runPack(pack, project, createdAt) {
  const redactions = new Set();
  const cases = [];
  for (const originalCase of pack.cases) {
    const item = await maybeExecuteCase(pack, originalCase);
...
module.exports = { combineReports, getProject, loadPack, renderMarkdown, runPack, validatePack };
